...
function initgame() {
    ...
    //отработка мыши, разберём во вторую очередь
...
function rule(i,j) {
    ...
    //return ruleUnivers( i, j, 110)
    return ruleUnivers( i, j, 22)
}
...
function ruleUnivers(i, j, number) {
    let num = number
    //раскодируем двоичный код
    let code = []
    ...
    let leftN = (game_field[i][ (j + LOG_WIDTH - 1) % LOG_WIDTH ] == ALIVE) ? 1 : 0
    let me = game_field[i][j] == ALIVE ? 1 : 0
    let rightN = game_field[i][ (j + 1) % LOG_WIDTH ] == ALIVE ? 1 : 0
      
    let resltNum = (leftN * 4 + me * 2 + rightN)
    
    let reslt = (code[resltNum] == 1) ? ALIVE : DEAD
